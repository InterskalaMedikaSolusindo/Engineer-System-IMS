...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS troubleshooting_files (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (guide_id) REFERENCES troubleshooting_guides(id)
  );
...
try {
  db.exec('ALTER TABLE spareparts ADD COLUMN prioritas_sparepart TEXT DEFAULT "generic"');
} catch (e) {
  // Column might already exist
}

try {
  db.exec('ALTER TABLE equipment ADD COLUMN tipe TEXT');
  db.exec('ALTER TABLE equipment ADD COLUMN prioritas_sparepart TEXT');
} catch (e) {
  // Column might already exist
}
...
try {
  db.exec('ALTER TABLE troubleshooting_guides ADD COLUMN isi_troubleshooting_user TEXT');
  db.exec('ALTER TABLE troubleshooting_guides ADD COLUMN file_path_user TEXT');
} catch (e) {
  // Column might already exist
}
...
app.post('/api/equipment', authenticateToken, requireRole(['Super Admin', 'Engineer']), (req, res) => {
  const { name, description, tipe, prioritas_sparepart } = req.body;
  const result = db.prepare('INSERT INTO equipment (name, description, tipe, prioritas_sparepart) VALUES (?, ?, ?, ?)').run(name, description, tipe, prioritas_sparepart);
...
app.put('/api/equipment/:id', authenticateToken, requireRole(['Super Admin', 'Engineer']), (req, res) => {
  const { name, description, tipe, prioritas_sparepart } = req.body;
  db.prepare('UPDATE equipment SET name = ?, description = ?, tipe = ?, prioritas_sparepart = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(name, description, tipe, prioritas_sparepart, req.params.id);
...
app.post('/api/spareparts', authenticateToken, requireRole(['Super Admin', 'Engineer']), upload.single('gambar'), (req, res) => {
  const { nama_sparepart, tipe, serial_number, brand, klasifikasi, equipment_id, stok_saat_ini, minimum_stock, kondisi, keterangan, prioritas_sparepart } = req.body;
  ...
  const result = db.prepare(`
    INSERT INTO spareparts (nama_sparepart, tipe, serial_number, brand, klasifikasi, equipment_id, stok_saat_ini, minimum_stock, restock_status, kondisi, gambar, keterangan, prioritas_sparepart, created_by, updated_by)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(nama_sparepart, tipe, serial_number, brand, klasifikasi, eqId, stok_saat_ini, minimum_stock, restock_status, kondisi, gambar, keterangan, prioritas_sparepart || 'generic', req.user.id, req.user.id);
...
app.put('/api/spareparts/:id', authenticateToken, requireRole(['Super Admin', 'Engineer']), upload.single('gambar'), (req, res) => {
  ...
  let query = `UPDATE spareparts SET nama_sparepart = ?, tipe = ?, serial_number = ?, brand = ?, klasifikasi = ?, equipment_id = ?, stok_saat_ini = ?, minimum_stock = ?, restock_status = ?, kondisi = ?, keterangan = ?, prioritas_sparepart = ?, updated_by = ?, updated_at = CURRENT_TIMESTAMP`;
  const params = [nama_sparepart, tipe, serial_number, brand, klasifikasi, eqId, stok_saat_ini, minimum_stock, restock_status, kondisi, keterangan, prioritas_sparepart || 'generic', req.user.id];
...
app.post('/api/troubleshooting', authenticateToken, requireRole(['Super Admin', 'Engineer']), upload.fields([{ name: 'file', maxCount: 1 }, { name: 'file_user', maxCount: 1 }]), (req, res) => {
  const { equipment_id, nama_trouble, isi_troubleshooting, isi_troubleshooting_user } = req.body;
  const files = req.files as { [fieldname: string]: Express.Multer.File[] };
  const file_path = files?.['file']?.[0] ? `/uploads/${files['file'][0].filename}` : null;
  const file_path_user = files?.['file_user']?.[0] ? `/uploads/${files['file_user'][0].filename}` : null;

  const eqId = (equipment_id && equipment_id !== 'null') ? parseInt(equipment_id) : null;
  const result = db.prepare(`
    INSERT INTO troubleshooting_guides (equipment_id, nama_trouble, isi_troubleshooting, file_path, isi_troubleshooting_user, file_path_user, created_by, updated_by)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(eqId, nama_trouble, isi_troubleshooting, file_path, isi_troubleshooting_user, file_path_user, req.user.id, req.user.id);
  
  logAudit(req.user.id, 'CREATE', 'Troubleshooting', result.lastInsertRowid as number, { nama_trouble });
  res.json({ id: result.lastInsertRowid });
});

app.put('/api/troubleshooting/:id', authenticateToken, requireRole(['Super Admin', 'Engineer']), upload.fields([{ name: 'file', maxCount: 1 }, { name: 'file_user', maxCount: 1 }]), (req, res) => {
  const { equipment_id, nama_trouble, isi_troubleshooting, isi_troubleshooting_user } = req.body;
  
  const eqId = (equipment_id && equipment_id !== 'null') ? parseInt(equipment_id) : null;
  let query = `UPDATE troubleshooting_guides SET equipment_id = ?, nama_trouble = ?, isi_troubleshooting = ?, isi_troubleshooting_user = ?, updated_by = ?, updated_at = CURRENT_TIMESTAMP`;
  const params = [eqId, nama_trouble, isi_troubleshooting, isi_troubleshooting_user, req.user.id];

  const files = req.files as { [fieldname: string]: Express.Multer.File[] };
  if (files?.['file']?.[0]) {
    query += `, file_path = ?`;
    params.push(`/uploads/${files['file'][0].filename}`);
  }
  if (files?.['file_user']?.[0]) {
    query += `, file_path_user = ?`;
    params.push(`/uploads/${files['file_user'][0].filename}`);
  }
  query += ` WHERE id = ?`;
  params.push(req.params.id);
...
app.delete('/api/troubleshooting/:id', authenticateToken, requireRole(['Super Admin', 'Engineer']), (req, res) => {
  ...
  db.prepare('DELETE FROM troubleshooting_files WHERE guide_id = ?').run(req.params.id);
  logAudit(req.user.id, 'DELETE', 'Troubleshooting', parseInt(req.params.id), {});
  res.json({ success: true });
});

// Troubleshooting Files
app.get('/api/troubleshooting/:id/files', authenticateToken, (req, res) => {
  const files = db.prepare('SELECT * FROM troubleshooting_files WHERE guide_id = ? ORDER BY created_at DESC').all(req.params.id);
  res.json(files);
});

app.post('/api/troubleshooting/:id/files', authenticateToken, requireRole(['Super Admin', 'Engineer']), upload.single('file'), (req, res) => {
  const { role, custom_name } = req.body;
  if (!req.file) return res.status(400).json({ error: 'No file uploaded' });
  
  const file_path = `/uploads/${req.file.filename}`;
  const file_name = custom_name || req.file.originalname;
  
  const result = db.prepare(`
    INSERT INTO troubleshooting_files (guide_id, role, file_name, file_path)
    VALUES (?, ?, ?, ?)
  `).run(req.params.id, role, file_name, file_path);
  
  res.json({ id: result.lastInsertRowid, file_name, file_path, role });
});

app.put('/api/troubleshooting/files/:id', authenticateToken, requireRole(['Super Admin', 'Engineer']), (req, res) => {
  const { file_name } = req.body;
  db.prepare('UPDATE troubleshooting_files SET file_name = ? WHERE id = ?').run(file_name, req.params.id);
  res.json({ success: true });
});

app.delete('/api/troubleshooting/files/:id', authenticateToken, requireRole(['Super Admin', 'Engineer']), (req, res) => {
  db.prepare('DELETE FROM troubleshooting_files WHERE id = ?').run(req.params.id);
  res.json({ success: true });
});
